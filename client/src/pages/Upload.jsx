...
const Upload = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.upload.uploadFile(file, title, classroomId);
  ...
  return (
    <div className="flex min-h-[calc(100vh-70px)] items-center justify-center bg-[radial-gradient(640px_380px_at_20%_16%,rgba(47,245,168,0.26),transparent_72%),linear-gradient(145deg,#27332e_0%,#1f2925_100%)] px-4 py-8">
      <div className="w-full max-w-2xl rounded-2xl border border-[#2ff5a838] bg-white/10 p-8 text-[#e8f2ed] shadow-2xl backdrop-blur">
        <h1 className="mb-2 text-3xl font-bold text-[#e8f2ed]">
          Upload Document
        </h1>
        ...
        <form onSubmit={handleSubmit} className="my-8">
          ...
          <div className="my-8 rounded-lg border border-[#2ff5a857] bg-linear-to-br from-[#2ff5a829] to-[#2ff5a814] p-6">
            ...
            <ul className="space-y-3">
              <li className="flex items-center gap-3 text-sm text-[#d8ebe3]">
                <Check size={18} className="text-[#8cf0c8]" /> Document analyzed
                by AI for summaries and tags
              </li>
              <li className="flex items-center gap-3 text-sm text-[#d8ebe3]">
                <Database size={18} className="text-[#8cf0c8]" /> Document
                stored on Duality for permanent access
              </li>
              <li className="flex items-center gap-3 text-sm text-[#d8ebe3]">
                <Lock size={18} className="text-[#8cf0c8]" /> Publication proof
                minted on Algorand blockchain
              </li>
              <li className="flex items-center gap-3 text-sm text-[#d8ebe3]">
                <Check size={18} className="text-[#8cf0c8]" /> Document
                auto-approved (you are a {user?.role})
              </li>
            </ul>
          </div>
